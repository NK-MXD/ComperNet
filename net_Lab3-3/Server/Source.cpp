...
void recv_message(char* message, int& len_recv) {
	...
	while (1) {
		while (1) {
			...
			if (recvfrom(m_ServerSocket, recv, MAX_UDP_LEN + 4, 0, (sockaddr*)&m_ClientAdress, &lentmp) == SOCKET_ERROR)continue;
			char ack_bag[3];
			int flag = 0;
			if (check_sum(recv, MAX_UDP_LEN + 4) == 0)
			{
				if ((unsigned char)recv[2] == unsigned char((last_order + 1) % ((int)UCHAR_MAX + 1)))
				{
					...
					sendto(m_ServerSocket, ack_bag, 3, 0, (sockaddr*)&m_ClientAdress, sizeof(m_ClientAdress));
					//send_ok_c++;
					//printf("%s%d\n", "已经确认", send_ok_c);
					//printf("%s%d\n\n", "send ack", recent_order - 1);
					cout << "已经确认接受第" << count++ << "个包" << endl;
					break;
				}
				char send[3];
				send[1] = ACK;
